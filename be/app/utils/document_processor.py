...
# Old-style DOC processing (for .doc files)
try:
    import subprocess

    HAS_ANTIWORD = (
        subprocess.run(["antiword", "-v"], capture_output=True).returncode == 0
    )
except:
    HAS_ANTIWORD = False
...
class DocumentProcessor:
    def __init__(self, data_dir: str = "data/thutuccongdan", openai_service=None):
        ...
        if CharacterTextSplitter:
            ...
            logger.info(
                f"🔧 [PROCESSOR] Initialized LangChain CharacterTextSplitter (fixed-size) with chunk_size={Config.CHUNK_SIZE}, overlap={Config.CHUNK_OVERLAP}"
            )
            ...
            logger.info(
                f"🔧 [PROCESSOR] Initialized RecursiveCharacterTextSplitter as fallback with chunk_size={Config.CHUNK_SIZE}, overlap={Config.CHUNK_OVERLAP}"
            )
        else:
            self.text_splitter = None
            logger.warning(
                "⚠️ [PROCESSOR] LangChain not available, falling back to simple chunking"
            )
        ...
        if MarkdownHeaderTextSplitter:
            ...
            logger.info(
                "🔧 [PROCESSOR] Initialized Markdown header splitter for context preservation"
            )
        else:
        ...
        self.processors = {
            ...
            ".doc": self._extract_doc_content,  # Separate handler for old .doc format
    ...
    def process_uploaded_file(
        ...
        try:
            ...
            if file_extension not in self.processors:
                logger.warning(
                    f"❌ [PROCESSOR] Unsupported file type: {file_extension}"
                )
                logger.info(
                    f"✅ [PROCESSOR] Supported types: {list(self.processors.keys())}"
                )
                return None
            ...
            logger.info(
                f"📝 [PROCESSOR] Content extracted, length: {len(content) if content else 0} characters"
            )

            if not content:
                logger.warning(
                    f"⚠️ [PROCESSOR] No content extracted from file: {filename}"
                )
                return None
            ...
            logger.info(
                f"📊 [PROCESSOR] Document stats - Title: '{doc['title']}', Content: {len(content)} chars"
            )
            return doc

        except Exception as e:
            logger.error(
                f"❌ [PROCESSOR] Failed to process file {filename}: {e}", exc_info=True
            )
            return None
    ...
    def _extract_docx_content(self, file_path: str) -> str:
        """Extract text content from DOCX file (Office Open XML format)"""
        if not Document:
            logger.error(
                "python-docx not available. Install python-docx to process DOCX files."
            )
            return ""
    ...
    def _extract_doc_content(self, file_path: str) -> str:
        """Extract text content from old-style DOC file (Word 97-2003 binary format)"""
        logger.info(f"📄 [DOC-PROCESSOR] Processing old-style .doc file")

        # Method 1: Try docx2txt (works for both .doc and .docx)
        if HAS_DOCX2TXT:
            try:
                logger.info(f"🔧 [DOC-PROCESSOR] Attempting extraction with docx2txt")
                content = docx2txt.process(file_path)
                if content and content.strip():
                    logger.info(
                        f"✅ [DOC-PROCESSOR] Successfully extracted with docx2txt ({len(content)} chars)"
                    )
                    return content.strip()
                else:
                    logger.warning(f"⚠️ [DOC-PROCESSOR] docx2txt returned empty content")
            except Exception as e:
                logger.warning(f"⚠️ [DOC-PROCESSOR] docx2txt failed: {e}")

        # Method 2: Try textract if available
        if HAS_TEXTRACT:
            try:
                logger.info(f"🔧 [DOC-PROCESSOR] Attempting extraction with textract")
                content = textract.process(file_path, method="antiword").decode(
                    "utf-8", errors="ignore"
                )
                if content.strip():
                    logger.info(
                        f"✅ [DOC-PROCESSOR] Successfully extracted with textract ({len(content)} chars)"
                    )
                    return content.strip()
            except Exception as e:
                logger.warning(f"⚠️ [DOC-PROCESSOR] textract failed: {e}")

        # Method 3: Try antiword command-line tool
        if HAS_ANTIWORD:
            try:
                logger.info(f"🔧 [DOC-PROCESSOR] Attempting extraction with antiword")
                import subprocess
                ...
                if result.returncode == 0 and result.stdout.strip():
                    logger.info(
                        f"✅ [DOC-PROCESSOR] Successfully extracted with antiword ({len(result.stdout)} chars)"
                    )
                    return result.stdout.strip()
                else:
                    logger.warning(
                        f"⚠️ [DOC-PROCESSOR] antiword failed with return code {result.returncode}"
                    )
            except Exception as e:
                logger.warning(f"⚠️ [DOC-PROCESSOR] antiword command failed: {e}")

        # Method 4: Try python-docx anyway (might work if it's actually a .docx with wrong extension)
        if Document:
            try:
                logger.info(
                    f"🔧 [DOC-PROCESSOR] Attempting extraction with python-docx (in case it's .docx with wrong extension)"
                )
                doc = Document(file_path)
                content = ""
                ...
                if content.strip():
                    logger.info(
                        f"✅ [DOC-PROCESSOR] File appears to be .docx format, extracted successfully ({len(content)} chars)"
                    )
                    return content.strip()
            except Exception as e:
                logger.debug(
                    f"📝 [DOC-PROCESSOR] python-docx failed (expected for true .doc files): {e}"
                )

        # Method 5: Extract as plain text (last resort - might get garbage)
        try:
            logger.info(
                f"🔧 [DOC-PROCESSOR] Last resort: attempting plain text extraction"
            )
            with open(file_path, "rb") as file:
                raw_content = file.read()
                # Try to extract readable ASCII/UTF-8 text from binary
                ...
                if content.strip() and len(content.strip()) > 50:
                    logger.warning(
                        f"⚠️ [DOC-PROCESSOR] Extracted text using plain text method (may contain artifacts): {len(content)} chars"
                    )
                    logger.warning(
                        f"⚠️ [DOC-PROCESSOR] Recommendation: Install 'textract' or 'antiword' for better .doc support"
                    )
                    return content.strip()
        except Exception as e:
            logger.error(f"❌ [DOC-PROCESSOR] Plain text extraction failed: {e}")

        logger.error(f"❌ [DOC-PROCESSOR] All extraction methods failed for .doc file")
        logger.error(
            f"💡 [DOC-PROCESSOR] Recommendation: Convert .doc file to .docx format in Microsoft Word"
        )
        logger.error(
            f"💡 [DOC-PROCESSOR] Or install: pip install docx2txt (already tried if available)"
        )
        return ""
    ...
    def _extract_image_content_with_llm(self, file_path: str) -> str:
        ...
        try:
            ...
            response = self.openai_service.chat_completion(
                messages, model="gpt-4o", max_tokens=1500
            )
    ...
    def _parse_markdown_content(self, content: str) -> tuple:
        ...
        if current_section.strip():
            sections.append(
                {"title": current_section_title, "content": current_section.strip()}
            )
    ...
    def chunk_documents(self, documents: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for doc in documents:
            ...
            full_content_chunks = self._split_text(
                doc["content"], ".md"
            )  # Assume markdown for legacy method
            for chunk_idx, chunk in enumerate(full_content_chunks):
                chunked_doc = {
                    "file_name": doc["file_name"],
                    "chunk_id": 9000
                    + chunk_idx,  # Different range for full document chunks
                    "content": chunk,
                    "title": doc["title"],
    ...
    def _split_text(self, text: str, file_type: str = None) -> List[str]:
        ...
        """
        logger.debug(
            f"📝 [CHUNKER] Starting LangChain fixed-size split for {len(text)} characters"
        )
        logger.debug(
            f"⚙️ [CHUNKER] Using CHUNK_SIZE={Config.CHUNK_SIZE}, OVERLAP={Config.CHUNK_OVERLAP}"
        )
        ...
        try:
            ...
            logger.warning(
                f"🔄 LangChain fixed-size chunking failed: {e}. Using simple fallback."
            )
        ...
        try:
            ...
            for doc in md_header_splits:
                ...
                if len(doc.page_content) <= self.chunk_size:
                    ...
                    chunk_with_context = (
                        header_context + doc.page_content
                        if header_context
            ...
            logger.info(
                f"🎯 [CHUNKER] Markdown header splitting generated {len(final_chunks)} chunks"
            )
            return final_chunks
    ...
    def _split_text_with_header_context(self, text: str) -> List[str]:
        ...
        logger.info(
            "📄 [CHUNKER] Using header context preservation for structured text"
        )
        ...
        try:
            ...
            logger.info(
                f"🎯 [CHUNKER] Header context splitting generated {len(enhanced_chunks)} chunks"
            )
    ...
    def _langchain_fixed_size_split(self, text: str, file_type: str) -> List[str]:
        ...
        try:
            ...
            # Validate chunks - if any chunk is still too large, force split it
            max_chunk_size = (
                Config.CHUNK_SIZE * 2
            )  # Allow 2x for safety, but enforce hard limit
            validated_chunks = []
            ...
            for chunk in chunks:
                if len(chunk) <= max_chunk_size:
                    ...
                    # Chunk is too large, force split it
                    logger.warning(
                        f"⚠️ Chunk too large ({len(chunk)} chars), force splitting..."
                    )
                    sub_chunks = self._force_split_large_chunk(chunk, Config.CHUNK_SIZE)
    ...
    def _langchain_markdown_fixed_split(self, text: str) -> List[str]:
        ...
        try:
            ...
            logger.info(
                f"📊 LangChain markdown fixed-split: {len(chunks)} chunks with headers"
            )
            for i, chunk in enumerate(chunks):
                logger.debug(f"   Chunk {i+1}: {len(chunk)} chars")
        ...
        try:
            ...
            while start < text_length:
                ...
                if end >= text_length:
                    ...
                    if (
                        len(chunk_text) < chunk_size * 0.3 and chunks
                    ):  # Less than 30% of chunk_size
                ...
                search_start = max(
                    start + chunk_size - chunk_overlap, start + chunk_size // 2
                )
    ...
    def _extract_document_headers(self, text: str) -> str:
        ...
        for line in lines[:20]:  # Check first 20 lines for main headers
            line = line.strip()
            if (
                line.startswith("#") and len(line.split()) <= 10
    ...
    def _standard_split(self, text: str) -> List[str]:
        ...
        try:
            ...
            logger.info(
                f"🎯 [CHUNKER] Standard splitting generated {len(chunks)} chunks"
            )
            ...
            if chunks:
                ...
                logger.debug(
                    f"📊 [CHUNKER] Chunk stats: avg={avg_size:.0f}, min={min_size}, max={max_size}"
                )
    ...
    def _extract_headers(self, text: str) -> List[dict]:
        ...
        for line in lines:
            ...
            # ALL CAPS headers
                ...
                re.match(r"^[A-Z][A-Z\s]{3,}$", line_stripped)
    ...
    def _find_relevant_headers(
        self, headers: List[dict], chunk_position: int
    ) -> List[dict]:
        ...
        for header in reversed(headers):  # Start from most recent
            if header["position"] <= chunk_position:
                ...
                for prev_header in reversed(headers):
                    if (
                        prev_header["position"] < header["position"]
                        and prev_header["level"] < current_level
                    ):
    ...
    def _force_split_large_chunk(self, text: str, target_size: int) -> List[str]:
        """
        Force split a large chunk into smaller pieces
        ...
        """
        logger.info(
            f"🔪 [CHUNKER] Force splitting large text: {len(text)} chars into ~{target_size} char chunks"
        )
        ...
        while start < len(text):
            ...
            if end < len(text):
                # Look for break points in a window
                search_start = end - 200  # Look back 200 chars
                search_end = end + 200  # Look ahead 200 chars

                # Try to find sentence break
                for i in range(
                    min(search_end, len(text)) - 1, max(search_start, start), -1
                ):
                    if (
                        text[i] in ".!?\n"
                        and i + 1 < len(text)
            ...
            if chunk:
                chunks.append(chunk)
                logger.debug(f"  Force chunk: {len(chunk)} chars")

            start = end

        logger.info(f"✅ [CHUNKER] Force split created {len(chunks)} chunks")
        return chunks
    ...
    def process_and_save_to_milvus(
        self, file_path: str, filename: str, milvus_service
    ) -> bool:
        """
        ...
        try:
            ...
            logger.info(
                f"📄 [MILVUS-PROCESSOR] Step 1: Extracting content from {filename}"
            )
            doc = self.process_uploaded_file(file_path, filename)
            if not doc:
                logger.error(
                    f"❌ [MILVUS-PROCESSOR] Failed to extract content from {filename}"
                )
                return False
            ...
            logger.info(
                f"✂️ [MILVUS-PROCESSOR] Step 2: Splitting content into chunks with header preservation"
            )
            chunks = self._split_text(doc["content"], doc["file_type"])
            logger.info(
                f"📊 [MILVUS-PROCESSOR] Generated {len(chunks)} chunks from content"
            )

            if not chunks:
                logger.warning(
                    f"⚠️ [MILVUS-PROCESSOR] No chunks generated from {filename}"
                )
                return False
            ...
            for chunk_idx, chunk in enumerate(chunks):
                ...
                logger.debug(
                    f"📝 [MILVUS-PROCESSOR] Chunk {chunk_idx}: {len(chunk)} characters"
                )

            logger.info(
                f"✅ [MILVUS-PROCESSOR] Prepared {len(milvus_docs)} documents for Milvus"
            )

            # Save to Milvus
            logger.info(f"💾 [MILVUS-PROCESSOR] Step 4: Saving to Milvus database")
            logger.info(
                f"🔌 [MILVUS-PROCESSOR] Milvus service available: {milvus_service is not None}"
            )

            success = milvus_service.insert_documents(milvus_docs)
            if success:
                logger.info(
                    f"🎉 [MILVUS-PROCESSOR] Successfully saved {len(milvus_docs)} chunks from {filename} to Milvus"
                )

                # Verify data was saved
                stats = milvus_service.get_collection_stats()
                logger.info(
                    f"📈 [MILVUS-PROCESSOR] Collection now has {stats} total entities"
                )
                return True
            else:
                logger.error(
                    f"❌ [MILVUS-PROCESSOR] Failed to save chunks from {filename} to Milvus"
                )
                return False

        except Exception as e:
            logger.error(
                f"💥 [MILVUS-PROCESSOR] Error processing and saving {filename} to Milvus: {e}",
                exc_info=True,
            )
            return False
